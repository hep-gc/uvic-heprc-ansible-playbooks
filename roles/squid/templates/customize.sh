...
setoption("cache_dir", "rock /var/cache/squid/squid${process_number} {{ CACHE_DIR }}")
...
insertline("^acl MAJOR_CVMFS", "acl B2DB dstdom_regex ^(belle2db.sdcc.bnl.gov)$ ")
insertline("^# http_access deny !RESTRICT_DEST", "http_access allow BELLEII")
insertline("^# http_access deny !RESTRICT_DEST", "http_access allow B2DB")
