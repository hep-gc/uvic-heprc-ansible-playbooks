...
def _device_type_and_path(SYS, device):
  _debug_log("Device: %s" % device)
  _debug_log("Device type: %s ; Should be string." % type(device))
  try:
      dv_array = device.split('/')
  except AttributeError:
      if "list" in str(type(device)):
          dv_array = device
      else:
          raise
  if len(dv_array) > 1 and dv_array[1] == 'dev':
    ...
    if SYS['zfs']:
      ...
      for line in lines:
        _debug_log("line in _device_type_and_path(): %s" % line)
        try:
            w = line.split()
        except AttributeError:
...
def _execute (SYS, cmd, error_return=False):
  ...
  try:
    ...
    stdout = to_text(stdout); stderr = to_text(stderr) 
    if p.returncode == 0:
      if error_return:
...
def _get_device_info(SYS, Disks):
  try:
    ...
    _debug_log("Attribute Error assigning bays=Disks.split(',').\n Disks: %s. Should be string" % Disks)
  Bn = int(bays[0])
  _debug_log('Bays: %s' % bays)
  ...
  if len(SYS['bays']) < 1:
    ...
    _debug_log('An: %s, Pn: %s' % (An, Pn))
    _debug_log('Sys bays: %s' % SYS['bays'])
    _debug_log('sys controllers: %s' % SYS['controllers'])
    _debug_log('our pn: %s' % SYS['controllers'][Pn])
    ...
    if disks[0] == '0' and FStype[:3] == 'zfs':
      for ix in range(len(bays)):
        ...
        if Ln in SYS['controllers'][Pn]['adapters'][An]['LDs'] and '%s,%s' % (RAIDlvl, SYS['bays'][Bn]['EnSn']) != SYS['controllers'][Pn]['adapters'][An]['LDs'][Ln]:
          ansible_module.fail_json(msg='volume_configuration(_megacli,%s): Can not create RAID 0 devices for ZFS (%s); requested disks are already configured differently (%s/%s).' % (_get_linenumber(), request, Ln, SYS['controllers'][Pn]['adapters'][An]['LDs'][Ln]), SYS=SYS)
      return False
...
def _mkfs (SYS, request, changed_state):
  _debug_log("Running _mkfs, request= %s" % request)
  RAIDlvl, Disks, VG, LV, GBs, FStype, Mountpoint = request
  changed = changed_state
  bays = Disks.split(',')
  ...
  if Mountpoint != '-':
    ...
    if Disks == '999':
      ...
      device = []
      mount = Mountpoint
#      for ix in range(int(sorted(bays)[-1])+1):
#        Bn=int(ix)
#        An = SYS['bays'][Bn]['An']; Pn = SYS['ctl_xref'][An];
#        Ln = int(SYS['bays'][Bn]['Disk']);
#        _debug_log("Bn: %s" % Bn)
#        _debug_log("Pn: %s, Ln: %s" % (Pn, Ln))
#        _debug_log("Got device: %s" % SYS['controllers'][Pn]['devs'][Ln])

#      ansible_module.fail_json(msg='Scheduled failure for debug purposes')
      for ix in range(len(bays)):
        Bn = int(bays[ix])
        _debug_log("Bn: %s" % Bn)
    ...
    _debug_log("SYS['fstab']: %s, Mount: %s" % (SYS['fstab'],mount))
    if not mount in SYS['fstab'] and 'zfs' not in FStype:
      if FStype == 'swap':
